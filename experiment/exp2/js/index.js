...
function generateStimulusList() {
  ...
  // Items 1..16
  var allItems = [];
  for (var i = 1; i <= 16; i++) allItems.push(i);

  // Fillers
  var grammaticalFillers = [];
  var ungrammaticalFillers = [];
  for (var j = 1; j <= 8; j++) {
    grammaticalFillers.push(8000 + j);    
    ungrammaticalFillers.push(9000 + j);  
  }

  // block assignment: pre-, exposure, and post-
  shuffle(allItems);
  var pre_list = allItems.slice(0, 4);
  var exposure_list = allItems.slice(4, 12);   // 8 critical items
  var post_list = allItems.slice(12, 16);

  // reshuffle within each block
  shuffle(pre_list);
  shuffle(exposure_list);
  shuffle(post_list);
  ...
  // generate pre-exposure block
  for (var i = 1; i <= 4; i++) {
    pre_block.push(1000 + 10 * pre_list[i - 1] + i);
  }
 

  // generate exposure critical items
  for (var i = 1; i <= 8; i++) {
    exp_criticals.push(1000 + 10 * exposure_list[i - 1] + 1);
  }

  // generate post-exposure block
  for (var i = 1; i <= 4; i++) {
    post_block.push(1000 + 10 * post_list[i - 1] + i);
  }
   shuffle(pre_block);
   shuffle(post_block);
  ...
  //////////////////////////////////////////////////////////////////
  // EXPOSURE BLOCK: 8 grammatical + 8 ungrammatical fillers,
  // with no two critical items adjacent
  //////////////////////////////////////////////////////////////////

  shuffle(grammaticalFillers);
  shuffle(ungrammaticalFillers);

  var exp_fillers = grammaticalFillers.concat(
    ungrammaticalFillers
  );
  shuffle(exp_fillers);

  // There are exp_fillers.length + 1 slots around the fillers
  // Example with 3 fillers: _ F _ F _ F _
  var slots = [];
  for (var i = 0; i <= exp_fillers.length; i++) {
    slots.push(i);
  }

  // choose 8 distinct slots for the 8 criticals
  var criticalSlots = getRandomNonDuplicateSelection(slots, exp_criticals.length, []);
  criticalSlots.sort(function(a, b) { return a - b; });

  shuffle(exp_criticals);
  ...
  for (var i = 0; i <= exp_fillers.length; i++) {
    ...
    // insert a critical before filler i if this slot was chosen
    if (criticalSlots.includes(i)) {
      exp_block.push(exp_criticals[criticalIndex]);
      criticalIndex++;
    }

    // then insert filler i (if it exists)
    if (i < exp_fillers.length) {
      exp_block.push(exp_fillers[i]);
    }
  }

var presentation_list = pre_block.concat(exp_block, post_block);
